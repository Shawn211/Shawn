...
play8='http://www.mfengshen.com/wapgame.php?sid=b463b27f990c98bbab7bb5489bc72764'
...
def login(player):
    lg0=requests.get(player,headers=headers4,proxies=proxies).content
    ...
    lg1=requests.get(url0,headers=headers4,proxies=proxies).content
    ...
    if re.search('欢迎回家',lg1):
        ...
        lg2=requests.get(url1,headers=headers4,proxies=proxies).content
        ...
        html=requests.get(gameurl,headers=headers4,proxies=proxies).content
        html=etn(html)
...
def job(gurl):
    html=requests.get(gurl,headers=headers4,proxies=proxies).content
    ...
    while 1:
        if re.search(r'♀.*性格:保守 特性:蜕皮<br />属性:',html):
            ...
            html=etn(html)
            gurl=searchurl(html,'挑战')
            html=requests.get(gurl,headers=headers5,proxies=proxies).content
            while 1:
